...
/**
* @package OAUTH
* @author Arvin Castro <arvin@sudocode.net>
* @date 16 April 2011
* @link http://sudocode.net/sources/includes/class-xhttp-php/
...
class xhttp {
	...
	public static function after_curl_execution(&$ch, &$response, &$requestData) {
		...
		if(isset($responseData['headers']['content-type']) && $responseData['headers']['content-type'] == 'application/json') {
	...
	public static function load($plugins) {
		...
		foreach($plugins as $name) if(false === array_search($name, self::$plugins)) {
			...
			# Our framework will give the class files for this class.
